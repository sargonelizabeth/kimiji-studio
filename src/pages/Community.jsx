...
export default function Community(){
  ...
  // 갤러리 헤더 업로드 버튼 → 로그인 가드 → 파일 선택기
  async function onUploadClick(){
    const { data:{ session } } = await supabase.auth.getSession();
    if(!session?.user){
      window.location.href = "/login.html?back=" + encodeURIComponent("/community.html");
      return;
    }
  ...
  return (
    <section className="community">
      {/* 시각적으로만 숨김: iOS 사파리에서 새 페이지로 열리는 문제 방지 */}
      <input
        ref={fileRef}
        type="file"
        accept="image/*"
...
function Card({ p, authed }){
      ...
      if(authed){
        ...
        if(uid){
          const { data } = await supabase
            .from("photo_likes")
            .select("photo_id")
            .eq("photo_id",p.id).eq("user_id",uid).maybeSingle();
          setLiked(!!data);
        } else setLiked(false);
      } else setLiked(false);
  ...
  async function postComment(e){
    e.preventDefault();
    try{
      const { data:s } = await supabase.auth.getSession(); const uid=s?.session?.user?.id; if(!uid || !text.trim()) return;
