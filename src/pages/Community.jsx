...
export default function Community(){
  ...
  React.useEffect(()=>{(async()=>{
    try{
      const { data } = await supabase.from("site_metrics")
        .select("prize_krw,cumulative_krw").eq("id",1).single();
  ...
  async function handlePick(e){
    ...
    e.target.value = "";           // 재선택 허용
    if(!file) return;

    const { data:{ session } } = await supabase.auth.getSession();
    if(!session?.user){
      alert("로그인이 필요합니다.");
      window.location.href = "/login.html?next=/community.html";
      return;
    }
    ...
    const { error: insErr } = await supabase.from("photo").insert({
      user_id: session.user.id, public_url: publicUrl, caption:""
    });
  ...
  return (
    <>
      ...
      {/* iOS에서 숨겨진 input 클릭 문제를 피하려고 화면 밖에 배치(visibility 아님) */}
      <input ref={fileRef} type="file" accept="image/*"
             style={{position:"fixed",left:"-9999px"}} onChange={handlePick} />
          ...
          <div className="gallery-head">
            ...
            <div className="right">
              <button className="upload" onClick={()=>{
                if(!user){ window.location.href="/login.html?next=/community.html"; return; }
      ...
      <style>{`
        .community{padding:24px 16px;color:#fff;background:#b58d59;}
        .kj-container{max-width:980px;margin:0 auto}
        .metrics{background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.12);border-radius:16px;padding:16px;display:grid;gap:12px;grid-template-rows:auto 1px auto;margin-bottom:16px}
        ...
        .empty{opacity:.9;padding:28px 0}
      `}</style>
    </>
  );
}
