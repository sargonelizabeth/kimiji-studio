...
const BUCKET = "photo";   // Storage 버킷 ID
const TABLE  = "photos";  // 사진 메타 테이블명(복수형)
...
export default function Community(){
  ...
  async function handlePick(e){
    ...
    if(!file.type?.startsWith("image/")){
      alert("이미지 파일만 업로드할 수 있어요."); e.target.value=""; return;
    }

    const { data:{ session } } = await supabase.auth.getSession();
    if(!session?.user){ alert("로그인 후 이용해주세요."); e.target.value=""; return; }
    ...
    await supabase.from(TABLE).insert({
      user_id: session.user.id, storage_path: key, public_url: pub.publicUrl, caption: ""
    });
  ...
  return (
    <section className="community">
      {/* iOS에서 capture 제거 → 사진 보관함 팝업 */}
      <input ref={fileRef} type="file" accept="image/*"
             style={{position:"absolute",width:1,height:1,opacity:0,pointerEvents:"none"}}
             onChange={handlePick} aria-hidden="true" tabIndex={-1} />
...
function Card({ p, authed }){
  ...
  async function refreshComments(){
    try{
      const { data } = await supabase.from("photo_comments")
        .select("id,content,created_at").eq("photo_id",p.id)
